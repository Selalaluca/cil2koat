...
このフォルダには、.NETアセンブリ内のCILを解析し、[KoAT](https://github.com/aprove-developers/KoAT2-Releases)向けの整数遷移系（`.koat`）へ変換するF#製の試作プログラムが含まれます。
...
- 到達不能ブロックの識別
- 遷移IRの整合性検証
- 同一staticメソッドへの末尾再帰を、戻り値の有無にかかわらず引数更新を伴う入口への遷移として変換

### 末尾再帰

同じstaticメソッドを末尾位置から直接呼び出す再帰に対応しています。戻り値を持つメソッドも、再帰呼び出しの結果をそのまま返す場合は扱えます。
...
再帰呼び出しは概念的に次の遷移になります。

```text
entry(x) -> recursive(x) [x > 0]
recursive(x) -> entry(x - 1)
```

同じ引数で再帰する場合は恒等自己ループとして残るため、停止性解析器が非停止の可能性を確認できます。戻り値そのものは停止性に影響しないため遷移系には保持しません。instance再帰、再帰結果を使って演算する非末尾再帰、相互再帰には対応していません。
...
F#コンパイラが生成した単一のクロージャを渡す`List.map`と`List.filter`を、入力リスト長を1ずつ減らす明示的なKoATループへ変換します。callbackは非循環で、副作用や別メソッド呼び出しを含まず、整数値だけをキャプチャする場合に限定します。現在は高階呼び出しが対象メソッドの末尾にあり、結果を直ちに返す場合だけ対応します。`filter`結果長の非決定性やcallbackの要素更新はまだ遷移系へ保持しません。条件を満たさないcallbackや結果の後続利用は、安全でない近似をせず変換を拒否します。

概念的な出力は次の形です。callbackの停止性はフロントエンドで構文的に確認し、有限走査の停止性はKoAT側へ残します。
