...
- 到達不能ブロックの識別
- 遷移IRの整合性検証
- 同一static `void`メソッドへの末尾再帰を、引数更新を伴う入口への遷移として変換

### 末尾再帰

同じstatic `void`メソッドを末尾位置から直接呼び出す再帰に対応しています。
...
同じ引数で再帰する場合は恒等自己ループとして残るため、停止性解析器が非停止の可能性を確認できます。現在は戻り値を持つ再帰、instance再帰、非末尾再帰、相互再帰には対応していません。
...
- `call`／`callvirt`は上記の文字列・リスト操作と、同一static `void`メソッドへの末尾再帰だけに対応します。一般のメソッド呼び出し、戻り値を持つ再帰、instance再帰、非末尾再帰、相互再帰、フィールド、一般の例外フロー、高階関数は未対応です。
